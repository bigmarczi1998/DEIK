...
class Station(City, Time):
    ...
    def __le__(self, other):
        if isinstance(other, Station):
            if int(self.get_hour()) == int(other.get_hour()):
                if int(self.get_min()) == int(other.get_min()):
                    return self.get_name() <= other.get_name()
                else:
                    return int(self.get_min()) <= int(other.get_min())
            else:
                return int(self.get_hour()) <= int(other.get_hour())
        else:
...
class IC:
    ...
    def input_stops(self, file_name):
        ...
        try:
            ...
            for line in file:
                strings = line.split(";")
                name = strings[2]
                times = strings[3].split(".")
                hour = int(times[0])
                min = int(times[1])
                s = Station(name, hour, min)
                stops.append(s)
    ...
    def __str__(self):
        ...
        return "{} ({}) from: {} to: {}\n".format(self.__name, self.__id, self.__arrival, self.__dest) + stops_str
    ...
    for i in ic.get_stops():
        if isinstance(i, Station):
            if i.get_name() not in menetrend:
                ...
                menetrend[i.get_name()] = [str(i.get_hour()) + ":" + str(i.get_min()) + " - " + ic.get_name()\
                                           + "(" + ic.get_id() + ")"]
            else:
                menetrend[i.get_name()].append(str(i.get_hour()) + ":" + str(i.get_min()) + " - " + ic.get_name()\
                                               + "(" + ic.get_id() + ")")
